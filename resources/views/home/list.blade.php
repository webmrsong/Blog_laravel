...
@section('info')
    <title>{{$field->cate_name}}-{{Config::get('web.web_site')}}</title>
...
<div class="newblog left">
   ...
   <p class="dateview"><span>发布时间：{{date('Y-m-d',$v->art_time)}}</span><span>作者：{{$v->art_editor}}</span><span>分类：[<a href="{{url('cate/'.$field->cate_id)}}">{{$field->cate_name}}</a>]</span></p>
    ...
    <ul class="nlist">
      ...
      <a title="{{$v->art_title}}" href="{{url('element/'.$v->art_id)}}" target="_blank" class="readmore">阅读全文>></a>
    </ul>
    <div class="line"></div>
    @endforeach
        <div class="page">
...
@if($cate)
   <div class="rnav">
      <ul>
          @foreach($cate as $k=>$c)
       <li class="rnav{{$k+1}}"><a href="{{url('cate/'.$field->cate_id)}}" target="_blank">{{$c->cate_name}}</a></li>
          @endforeach
     </ul>      
    </div>
    @endif
